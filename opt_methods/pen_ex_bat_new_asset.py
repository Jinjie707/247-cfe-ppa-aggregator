...
class PenExBatNewAsset(OptimizationCase):
    """
    Optimization Case 11: With Existing Battery and Penalty, optimizing New Asset.
    ...
    def construct(self):
        ...
        # Rule 12: SOC constraint, battery energy level is bounded by battery capacity
        def rule_12(model, t):
            return (model.soc[t] <= bat_capacity)
        model.cst12 = Constraint(T, rule = rule_12)

        # Rule 13: Force the initial status of the battery to be empty        
        def rule_13(model):
    ...
    def solve(self, optimizer):
        ...
        if results.solver.termination_condition in [
            ...
            # Deduce new gen beta
            for i in range(8760):
                diff = v_total[i] - temp_total[i] + u_c[i] - u_d[i]*bat_rte - v_buy[i]
                if new_prod[i] == 0:
                    new_beta.append(0)
                else:
