...
class PenNewBatNewAsset(OptimizationCase):
    """
    Optimization Case 9: OPtimizing for New Battery and New Asset with Pen.

    Attributes:
        Attributed initialized from parent class.
        Penalty, New Gen and New Bat attributes to be initialized seperately to remind this case.
    """
    ...
    def construct(self):
        ...
        model.v_total = Var(T, within=NonNegativeReals, bounds = (0, M), initialize = 0) # total = production + buy + discharge
        ...
        def rule_4(model, t):
            return (model.u_c[t] <= sum(model.beta[v,t]*productions[v][t] for v in p_N) 
    ...
    def solve(self, optimizer):
        ...
        if results.solver.termination_condition in [
            ...
            self.processed_inputs['Existing Battery'] = (True, self.existing_bat) # Stupidly setting back to the list
    ...
    def process_result(self):
        ...
        v_renewable = np.array(v_total) - np.array(v_buy)
